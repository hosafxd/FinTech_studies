---
title: What Does AI Actually Learn?
category: Machine Learning & AI in Finance
topics: [interpretability, feature importance, overfitting, model diagnostics]
source_notebook: what_does_ai_actually_learn.ipynb
---
...
> **Category:** Machine Learning & AI in Finance  
> **Topics:** interpretability, feature importance, overfitting, model diagnostics  
> **Source notebook:** `what_does_ai_actually_learn.ipynb`

## Description

Opens the black box of ML models trained on financial data. Uses interpretability tools to show what features a model actually relies on — and how often it learns noise instead of signal.

## When this material is useful

- Diagnosing whether a model is exploiting real signal or overfitting noise
- Building intuition before deploying any ML model to live trading
- Choosing between simple and complex models based on what signal exists
...
- Title: 3D Feature Space: Raw vs Return-Colored Points with Trading Signal
- Traces (4):
  - `trace0` (scatter3d) mode=markers
  - `Positive Return` (scatter3d) mode=markers
  - `Negative Return` (scatter3d) mode=markers
  - `Trading Signal` (surface)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Random Variable Examples
- X axis: Dice Value
- Y axis: Probability
- Traces (2):
  - `Dice Roll Distribution` (bar)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Empirical vs Theoretical Dice Roll Distributions
- X axis: Dice Value
- Y axis: Probability
- Traces (7):
  - `Empirical (5 rolls)` (bar)
  - `Theoretical` (scatter)
  - `Empirical (1000 rolls)` (bar)
  - `Theoretical` (scatter)
  - `Empirical (10000 rolls)` (bar)
  - `Theoretical` (scatter)
  - `Theoretical` (bar)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Age vs Height Example
- X axis: Age (years)
- Y axis: Height (inches)
- Traces (2):
  - `Age-Height Data` (scatter) mode=markers, n=7
  - `Same Age, Different Heights` (scatter) mode=markers, n=2
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Distribution of Mid Prices
- X axis: Mid Price
- Y axis: Frequency
- Traces (1):
  - `Mid Price Distribution` (histogram)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Empirical vs Theoretical Probability of Even Dice Rolls
- X axis: Outcome
- Y axis: Probability
- Traces (7):
  - `Empirical (5 rolls)` (bar) n=2
  - `Theoretical` (scatter) n=2
  - `Empirical (1000 rolls)` (bar) n=2
  - `Theoretical` (scatter) n=2
  - `Empirical (10000 rolls)` (bar) n=2
  - `Theoretical` (scatter) n=2
  - `Theoretical` (bar) n=2
