---
title: Markov Chains — State Transitions in Quantitative Finance
category: Stochastic Calculus & Math Foundations
topics: [Markov chain, transition matrix, stationary distribution, regime switching]
source_notebook: markov_chains.ipynb
---

# Markov Chains — State Transitions in Quantitative Finance

> **Category:** Stochastic Calculus & Math Foundations  
> **Topics:** Markov chain, transition matrix, stationary distribution, regime switching  
> **Source notebook:** `markov_chains.ipynb`

## Description

Discrete-state Markov processes applied to finance: credit-rating migration, regime-switching models, and discrete approximations of continuous dynamics. Computes transition matrices, stationary distributions, and expected hitting times.

## When this material is useful

- Modeling credit-rating transitions or market regimes (bull/bear/sideways)
- Computing long-run probabilities from a transition matrix
- Approximating a continuous process with a discrete chain for tractable simulation
- Distinguishing memoryless (Markov) from path-dependent processes
...
- Title: Drifting Return Distribution with Accumulated Samples
- Traces (2):
  - `Drifting Normal` (scatter) mode=lines
  - `Sample Distribution` (histogram) n=10
...
- Traces (2):
  - `Probability` (bar)
  - `Dice Rolls` (scatter) mode=lines+markers
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Example Loan Portfolio Delinquency State
- Traces (1):
  - `trace0` (pie)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Independent Model Shows Impossible Transitions
- X axis: Delinquency Status
- Y axis: Proportion of Loans
- Traces (12):
  - `Current` (bar) n=1
  - `30-59 Days Late` (bar) n=1
  - `60-89 Days Late` (bar) n=1
  - `90+ Days Late` (bar) n=1
  - `Current` (bar) n=1
  - `30-59 Days Late` (bar) n=1
  - `60-89 Days Late` (bar) n=1
  - `90+ Days Late` (bar) n=1
...
- Y axis: Proportion of Loans
- Traces (4):
  - `Current` (scatter) n=50
  - `30-59 Days Late` (scatter) n=50
  - `60-89 Days Late` (scatter) n=50
  - `90+ Days Late` (scatter) n=50
...
- Y axis: Probability
- Traces (6):
  - `State A (Empirical)` (scatter)
  - `State A (True)` (scatter) n=2
  - `State B (Empirical)` (scatter)
  - `State B (True)` (scatter) n=2
  - `State C (Empirical)` (scatter)
  - `State C (True)` (scatter) n=2
