---
title: Why Monte Carlo Simulation Works
category: Options Pricing & Volatility
topics: [Law of Large Numbers, Central Limit Theorem, convergence, standard error]
source_notebook: why_monte_carlo_simulation_works.ipynb
---
...
> **Category:** Options Pricing & Volatility  
> **Topics:** Law of Large Numbers, Central Limit Theorem, convergence, standard error  
> **Source notebook:** `why_monte_carlo_simulation_works.ipynb`

## Description

The mathematical justification for Monte Carlo: the Law of Large Numbers guarantees convergence to the true expectation, and the CLT tells you the rate. Includes practical guidance on sample-size selection and confidence intervals.

## When this material is useful

- Explaining why a simulated mean approximates the true expected value
- Computing the standard error of a Monte Carlo estimate
- Picking the number of paths needed for a target accuracy
- Defending or attacking the validity of a simulation-based result
...
- Title: Evolving Distribution Over Time
- Traces (2):
  - `Empirical Distribution` (scatter) mode=lines
  - `True Distribution` (scatter) mode=lines
...
- Y axis: Density
- Traces (2):
  - `Empirical Distribution` (histogram) n=0
  - `Theoretical Distribution` (scatter)
...
- Traces (3):
  - `Theoretical Distribution` (scatter)
  - `Theoretical Mean` (scatter) n=2
  - `Empirical Mean` (scatter) n=2
...
```
c:\Users\Roman\AppData\Local\Programs\Python\Python310\lib\site-packages\numpy\_core\fromnumeric.py:4318: RuntimeWarning:
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Monte Carlo Convergence for Dice Rolls
- Traces (4):
  - `Sample Mean` (scatter)
  - `True Mean` (scatter) n=2
  - `Sample Variance` (scatter)
  - `True Variance` (scatter) n=2
...
```
Theoretical mean: $325.00
```

**Out [6].2:**

**Interactive Plotly chart** (rendered live in the notebook).
- Title: Monte Carlo Convergence for Game Outcomes
- Traces (2):
  - `Sample Mean` (scatter)
  - `True Mean` (scatter) n=2
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: House Wealth Paths with Different Entry Fees
- X axis: Number of Games Played
- Y axis: Wealth ($)
- Traces (3):
  - `$330 Entry Fee` (scatter)
  - `$325 Entry Fee` (scatter)
  - `$320 Entry Fee` (scatter)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Convergence of Win Probability Estimate
- X axis: Number of Simulations
- Y axis: Probability of Winning
- Traces (1):
  - `Estimated Probability of Winning` (scatter) mode=lines
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Wealth Paths (Probability of Reaching Target: 46.1%)
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: European Call Option Price: Monte Carlo vs Black-Scholes
- X axis: Time Steps
- Y axis: Stock Price ($)
- Traces (102):
  - `trace0` (scatter) mode=lines
