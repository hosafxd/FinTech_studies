---
title: Itô Integration
category: Stochastic Calculus & Math Foundations
topics: [Itô integral, stochastic integration, Riemann-Stieltjes, quadratic variation]
source_notebook: ito_integration.ipynb
---
...
> **Category:** Stochastic Calculus & Math Foundations  
> **Topics:** Itô integral, stochastic integration, Riemann-Stieltjes, quadratic variation  
> **Source notebook:** `ito_integration.ipynb`

## Description

Builds the Itô integral from first principles: why it must be defined as a forward (non-anticipating) Riemann sum, how it differs from the Stratonovich integral, and what the quadratic variation of Brownian motion implies.

## When this material is useful

- Computing ∫ f(W_t) dW_t and understanding why ordinary calculus rules fail
- Explaining the difference between Itô and Stratonovich conventions
- Justifying the appearance of σ²/2 corrections in geometric Brownian motion
- Pricing path-dependent derivatives that require stochastic integrals
...
- Title: Itô Integration as a Lifted Path onto a Surface Weight
- Traces (10):
  - `Weighting Function f(t,W) = t` (surface)
  - `Lifted Path` (scatter3d) mode=lines
  - `Brownian Path` (scatter3d) mode=lines
  - `trace3` (scatter3d) mode=lines, n=2
  - `trace4` (scatter3d) mode=lines, n=2
  - `trace5` (scatter3d) mode=lines, n=2
  - `trace6` (scatter3d) mode=lines, n=2
  - `trace7` (scatter3d) mode=lines, n=2
  - `trace8` (scatter3d) mode=lines, n=2
  - `Itô Integral Value` (scatter3d) mode=lines
...
_[Interactive Jupyter widget — not renderable in markdown]_

```
interactive(children=(IntSlider(value=6, description='Number of rectangles:', layout=Layout(width='400px'), ma…
```

**Out [89].2:**

```
<function __main__.plot_riemann_sum(n)>
```
...
- Y axis: f(x)
- Traces (1):
  - `Standard Normal PDF` (scatter) mode=lines
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Brownian Motion and Brownian Increments
- Y axis: W(t)
- Traces (10):
  - `Sample Path 1` (scatter) mode=lines
  - `Increments 1` (scatter) mode=lines
  - `Sample Path 2` (scatter) mode=lines
  - `Increments 2` (scatter) mode=lines
  - `Sample Path 3` (scatter) mode=lines
  - `Increments 3` (scatter) mode=lines
  - `Sample Path 4` (scatter) mode=lines
  - `Increments 4` (scatter) mode=lines
  - `Sample Path 5` (scatter) mode=lines
  - `Increments 5` (scatter) mode=lines
...
- Y axis: Differential
- Traces (2):
  - `Empirical Average` (scatter)
  - `Theoretical (Itô's Lemma)` (scatter)
...
- Traces (139):
  - `trace0` (scatter) mode=lines
  - `trace1` (scatter) mode=lines
  - `trace2` (scatter) mode=lines
  - `trace3` (scatter) mode=lines
  - `trace4` (scatter) mode=lines
  - `trace5` (scatter) mode=lines
  - `trace6` (scatter) mode=lines
  - `trace7` (scatter) mode=lines
  - `trace8` (scatter) mode=lines
  - `trace9` (scatter) mode=lines
  - `trace10` (scatter) mode=lines
  - `trace11` (scatter) mode=lines
  - `trace12` (scatter) mode=lines
  - `trace13` (scatter) mode=lines
  - `trace14` (scatter) mode=lines
  - `trace15` (scatter) mode=lines
  - `trace16` (scatter) mode=lines
  - `trace17` (scatter) mode=lines
  - `trace18` (scatter) mode=lines
  - `trace19` (scatter) mode=lines
  - ... and 119 more traces
