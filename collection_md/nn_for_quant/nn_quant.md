---
title: Neural Networks for Quant Finance
category: Machine Learning & AI in Finance
topics: [neural networks, time series, feature engineering, quant ML]
source_notebook: nn_quant.ipynb
---
...
> **Category:** Machine Learning & AI in Finance  
> **Topics:** neural networks, time series, feature engineering, quant ML  
> **Source notebook:** `nn_quant.ipynb`

## Description

Applies neural networks to quantitative-finance problems: feature engineering for time-series, train/val/test splits that respect time ordering, and architectures suited to financial data.

## When this material is useful

- Avoiding lookahead bias in train/test splits
- Choosing an architecture (MLP, LSTM, CNN) for a financial prediction task
- Engineering features (lags, returns, volatility) for an NN input
- Benchmarking an NN against simple statistical baselines
...
- Y axis: Error
- Traces (7):
  - `Convex Error` (scatter) mode=lines
  - `optimal convex` (scatter) mode=markers, n=1
  - `trace2` (scatter) mode=lines, n=2
  - `Non-Convex Error` (scatter) mode=lines
  - `Local Minima` (scatter) mode=markers
  - `optimal non convex` (scatter) mode=markers, n=1
  - `trace6` (scatter) mode=lines, n=2
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Out-of-sample Points Added: <b>0</b>
- X axis: Input x
- Y axis: Target y
- Traces (12):
  - `trace0` (scatter) mode=markers
  - `Underfit` (scatter) mode=lines
  - `trace2` (scatter) mode=markers, n=0
  - `trace3` (scatter) mode=lines, n=0
  - `trace4` (scatter) mode=markers
  - `Overfit` (scatter) mode=lines
  - `trace6` (scatter) mode=markers, n=0
  - `trace7` (scatter) mode=lines, n=0
  - `trace8` (scatter) mode=markers
  - `Robust` (scatter) mode=lines
  - `trace10` (scatter) mode=markers, n=0
  - `trace11` (scatter) mode=lines, n=0
...
- Y axis: Loss
- Traces (3):
  - `Training Loss` (scatter) mode=lines+markers
  - `Validation Loss` (scatter) mode=lines+markers
  - `Stop Training` (scatter) mode=lines, n=2
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Stock Path: In-sample Fit vs Out-of-sample Reality
- Traces (5):
  - `ABM True Path` (scatter) mode=lines+markers
  - `Polynomial Fit (Train)` (scatter) mode=lines
  - `Polynomial Fit (OOS)` (scatter) mode=lines
  - `Polynomial Extrapolation (Future)` (scatter) mode=lines
  - `trace4` (scatter) mode=lines, n=2
...
- Title: Dynamic Stock Return Distribution (left) vs Fixed Dice Roll Uniform (right)
- Traces (4):
  - `Variance Path` (scatter) mode=lines
  - `Varying Normal` (scatter) mode=lines
  - `NVDA Distribution` (histogram) n=30
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: ReLU Piecewise Linear Approximation Converging to Conditional Expectation
- X axis: x
- Y axis: y
- Traces (3):
  - `Data` (scatter) mode=markers
  - `True Function` (scatter) mode=lines
  - `Neural Network` (scatter) mode=lines
...
**Interactive Plotly chart** (rendered live in the notebook).
- Title: Model Comparison: Nearly Solved vs Unsolved Problems
- Traces (8):
  - `Recognized (Yes)` (scatter) mode=markers
  - `Not Recognized (No)` (scatter) mode=markers
  - `trace2` (contour)
  - `ABM True Path` (scatter) mode=lines+markers
  - `Polynomial Fit (Train)` (scatter) mode=lines
  - `Polynomial Fit (OOS)` (scatter) mode=lines
  - `Polynomial Extrapolation (Future)` (scatter) mode=lines
  - `trace7` (scatter) mode=lines, n=2
...
- Y axis: Call Price
- Traces (2):
  - `Analytical Black-Scholes` (scatter) mode=lines
  - `Neural Network Approx` (scatter) mode=lines
